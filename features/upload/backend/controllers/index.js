...
module.exports = function (controller, component, app) {
    ...
    controller.upload = function (req, res) {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            let tmpFolder = standardPath + '/fileman/tmp/' + files["files[]"].name;
                        ...
                        if (!err) {
                            ...
                            if (number) {
                                checkFileExist(noExt.replace(/\(\d\)$/, ''), 1, ext).then(function (result) {
                                    fs.copyFileSync(files["files[]"].path, result, function (err) {
                            ...
                            fs.copyFileSync(files["files[]"].path, destination, function (err) {
                                if (err) logger.error(err);
                            });
                        };
    ...
    controller.thumb = function (req, res) {
        ...
        if (req.permissions.indexOf(permissionManageAll) === -1 &&
            ...
            if (fs.existsSync(tmpFolder + '/' + filename)) {
                ...
                // Create thumbnail
                fs.copyFileSync(standardPath + filePath, tmpFolder + '/' + filename, function (err) {
                    if (err) logger.error(err);
                }).then(function () {
                    try {
                        thum.thumbnail({
                            src: standardPath + filePath,
